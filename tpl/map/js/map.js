...
var map = {
	...
	*/
	render_types_list : function(options) {
		...
		// types list render
			const types_rows_node = this.render_types_rows({
				types_rows	: types_rows,
				coins_rows	: coins_rows,
				item_type	: item_type
			})
			fragment.appendChild(types_rows_node)
	...
	/**
	* RENDER_TYPES_ROWS
	* Render types nodes for list
	* @param object options
	* {
	* 	types_rows: array of objects (table catalog records)
	* 	coins_rows: array of objects (table coins records)
	* 	item_type: string (findspot|mint|hoard)
	* }
	* @return DocumentFragment
	*/
	render_types_rows : function(options) {

		// options
			const types_rows	= options.types_rows || []
			const coins_rows	= options.coins_rows || []
			const item_type		= options.item_type || null

		// DocumentFragment
			const fragment = new DocumentFragment()

		// empty case
			if (types_rows.length<1) {
				// debug info
				if(SHOW_DEBUG===true) {
