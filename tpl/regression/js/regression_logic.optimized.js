...
export const regression_logic = {
 ...
 /**
     ...
     *    /**
     * Calculates bootstrap confidence bands for obverse and reverse.
     * Higher B values increase accuracy but take more time.
     * Caches the promise to prevent redundant parallel executions.
     *
     * @param {number} B Number of bootstrap iterations
     * @returns {Promise<Object>}
     */
    get_bootstrap_bands: function(B = 2000) {
        ...
        bootstrap_promise = this.get_log_regression_coefficients().then((fit) => {
            const {
                alphaA,
                betaA,
                alphaR,
            ...
            // Pre-calculate logarithms of inputs for substantial speedup in the loop
            const logIR = IR_Ant_filtrat.map(v => Math.log(v));
            const logDA = D_A_Ant_filtrat.map(v => Math.log(v));
            const logDR = D_R_Ant_filtrat.map(v => Math.log(v));

            const coeffsA = new Array(B);
            const coeffsR = new Array(B);

            // IR where the central model predicts exactly 1 die
            const irAtOneA = Math.exp(-alphaA / betaA);
            const irAtOneR = Math.exp(-alphaR / betaR);

            const predAtOneA = new Array(B);
            const predAtOneR = new Array(B);

            for (let b = 0; b < B; b++) {
                // Efficient bootstrap sampling
                const Xb   = new Array(n);
                const YA_b = new Array(n);
                const YR_b = new Array(n);
                ...
                coeffsA[b] = { a: fitA_b.a, b: fitA_b.b };
                coeffsR[b] = { a: fitR_b.a, b: fitR_b.b };

                // Bootstrap distribution for "1 die" case
                predAtOneA[b] = this.predict_potential(irAtOneA, fitA_b.a, fitA_b.b);
                predAtOneR[b] = this.predict_potential(irAtOneR, fitR_b.a, fitR_b.b);
            }
            ...
            if (SHOW_DEBUG === true) {
                console.log("---> bootstrap data ready (B=" + B + ")");
            }
     ...
     * Calculates the CI on-the-fly from bootstrap coefficients.
     *
     * @param {number} ir
     * @param {Array<Object>} coeffs Array of {a, b} bootstrap coefficients
     * @returns {{lwr:number, med:number, upr:number}|null}
     */
    get_bootstrap_interval_for_ir: function(ir, coeffs) {
        if (!Number.isFinite(ir) || ir <= 0 || !Array.isArray(coeffs) || coeffs.length === 0) {
            return null;
        }

        const predictions = coeffs.map(c => this.predict_potential(ir, c.a, c.b));
        return this.bootstrap_band_from_vector(predictions);
    },
        ...
        return Promise.all([
            ...
            const vect_tipos_with_ci = vect_tipos.map(obj => {
                ...
                const ci = (obj.approx < 1)
                    ? bootstrap.oneDieA
                    : this.get_bootstrap_interval_for_ir(obj.ir, bootstrap.coeffsA);
   ...
   plot_points_regression_rev: function(parsed_data, regression_model_chart_container) {
        ...
        return Promise.all([
            ...
            const vect_tipos_with_ci = vect_tipos.map(obj => {
                ...
                const ci = (obj.approx < 1)
                    ? bootstrap.oneDieR
                    : this.get_bootstrap_interval_for_ir(obj.ir, bootstrap.coeffsR);
